    ...
    const permanentDeleteStuff = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                axios.delete(`http://localhost:8000/user/permanent/${id}`, config)
                .then(() => {
                    setTrash(trash.filter(user => user.id !== id));
                    Swal.fire(
                        'Berhasil!',
                ...
                <div className="m-5 pb-10 pt-10">
                    ...
                    <div className="flex mt-4 md:mt-6">
                        <table className="min-w-full text-left text-sm font-light">
                            ...
                            <tbody>
                                {trash.map((trash, id) => (
                                    <tr key={trash.id} className="border-b dark:border-neutral-500">
                                        <td className="whitespace-nowrap px-6 py-4">{id+1}</td>
                                        <td className="whitespace-nowrap px-6 py-4">{trash.username}</td>
                                        <td className="whitespace-nowrap px-6 py-4">{trash.email}</td>
                                        <td className="whitespace-nowrap px-6 py-4">{trash.password}</td>
                                        <td className="whitespace-nowrap px-6 py-4">{trash.role}</td>
                                        {/* {
                                            stuff.stock? (
                                        ...
                                        <td className="border px-4 py-2">
                                            <button className="mr-2 text-green-500 hover:text-green-700" onClick={() => restoreStuff(trash.id)}>
                                                <MdRestore className="w-6 h-6" />
                                            </button>
                                            <button className="text-red-500 hover:text-red-700" onClick={() => permanentDeleteStuff(trash.id)}>
